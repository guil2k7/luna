...
uint8_t* game::GAME_ADDRESS = nullptr;
...
static void removeWriteProtections() {
    // .data
    virtualModifyProtection(GAME_ADDRESS + 0x67A000, 0x6B2D84 - 0x67A000, PROTECTION_READ | PROTECTION_WRITE);

    // .got
    virtualModifyProtection(GAME_ADDRESS + 0x66E4D0, 0x679FF3 - 0x66E4D0, PROTECTION_READ | PROTECTION_WRITE);

    // .bss
    virtualModifyProtection(GAME_ADDRESS + 0x6B2DC0, 0xA98FEF - 0x6B2DC0, PROTECTION_READ | PROTECTION_WRITE);

    // .text
    virtualModifyProtection(GAME_ADDRESS + 0x1A1780, 0x5E84E7 - 0x1A1780, PROTECTION_READ | PROTECTION_WRITE | PROTECTION_EXEC);
}
