#include "SAMP.h"
#include <cstring>

void RakNet::SAMP::Obfuscate(uint8_t* dest, uint8_t const* src, size_t len, uint32_t salt) {
    static const uint8_t TABLE[256] = {
        
    };

    uint8_t checksum = 0;
    uint8_t byte;
    uint8_t nextByte = src[0];

    for (size_t i = 0; i < len; ++i) {
        byte = nextByte;
        checksum ^= byte & 0xAA;
        nextByte = src[i + 1];

        if ((i & 1) == 0)
            dest[i + 1] = TABLE[byte];
        else
            dest[i + 1] = TABLE[byte] ^ static_cast<uint8_t>(salt ^ 0xCC);
    }

    dest[0] = checksum;
}

bool RakNet::SAMP::Deobfuscate(uint8_t* dest, uint8_t const* src, size_t len, uint32_t salt) {
    static const uint8_t TABLE[256] = {
        
    };

    uint8_t checksum = 0;
    uint8_t checksumExpected = src[0];
    uint8_t byte;

    for (size_t i = 1; i < len; ++i) {
        byte = src[i];

        if ((i & 1) == 0)
            byte ^= static_cast<uint8_t>(salt ^ 0xCC);

        byte = TABLE[byte];
        checksum ^= byte & 0xAA;

        dest[i - 1] = byte;
    }

    if (checksum != checksumExpected)
        return false;

    return true;
}
